...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    NodeStruct *node = myFileSystem.nodes[fi->fh]; // fi->fh es la "m"? el i-nodo
    ...
    // hacen falta los sync y updates? no hemos cambiado nada del i-nodo....
...
static int my_unlink(const char *path) 
{
    ...
    //hace falta el sync? no lo piden en el pseudocodigo // q hace sync? pq antes de los updates?
    sync();
    ...
    // Liberar la memoria del nodo-i y actualizar la tabla ???? con tabla se refieren a mapa de bits?????
