...
vector<pair<pt, int>> teamA, teamB;
...
bool resuelveCaso() {
    ...
    vector<pair<pt, int>> towers; // x,y poder
    
    pt point;
    int p;
    for (int i = 0; i < nTowers; i++) {
        cin >> point.x >> point.y >> p;
        towers.push_back({ point, p });
    }

    int p1, p2, r;
    cin >> p1 >> p2 >> r;

    line lDivision(towers[p1-1].first, towers[p2-1].first);

    for (int i = 0; i < towers.size(); i++) {
        if (towers[i].second > 0) {
            if (lDivision.side(towers[i].first) > 0) {
                teamA.push_back(towers[i]);
            }
            else if (lDivision.side(towers[i].first) < 0) {
                teamB.push_back(towers[i]);
            }
        }
    }

    vector<pair<pt, int>> smallTeam;
    vector<pair<pt, int>> bigTeam;
    ...
    grafo.assign(towers.size(), vector<int>());
    for (int i = 0; i < smallTeam.size(); i++) {
        for (int j = 0; j < bigTeam.size(); j++) {
            if ((dist(smallTeam[i].first, bigTeam[j].first) <= r) && smallTeam[i].second > bigTeam[j].second) {
                grafo[i].push_back(j+M);
                grafo[j+M].push_back(i);
            }
        }
    }
